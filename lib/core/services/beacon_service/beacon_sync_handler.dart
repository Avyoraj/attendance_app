...
class BeaconSyncHandler {
  ...
  void init(
      BeaconCooldownManager cooldownManager, BeaconStateManager stateManager) {
  ...
  Future<Map<String, dynamic>> syncStateFromBackend(String studentId) async {
    try {
      _logger.i(
          '🔄 Syncing attendance state from backend for student: $studentId');

      // Fetch today's attendance from backend
      final result =
          await _httpService.getTodayAttendance(studentId: studentId);
      ...
      _logger.i(
          '📥 Received ${attendance.length} attendance records from backend');
      ...
      for (var record in attendance) {
        ...
        if (status == 'confirmed') {
          ...
          syncedCount +=
              _processConfirmedRecord(studentId, classId, record) ? 1 : 0;
        } else if (status == 'provisional') {
          // Process provisional attendance
          syncedCount +=
              _processProvisionalRecord(studentId, classId, record) ? 1 : 0;
        } else if (status == 'cancelled') {
          // Process cancelled attendance
          syncedCount +=
              _processCancelledRecord(studentId, classId, record) ? 1 : 0;
        }
      }
  ...
  bool _processConfirmedRecord(
      String studentId, String classId, Map<String, dynamic> record) {
    ...
    if (cooldownInfo != null && cooldownInfo['isActive'] == true) {
      _logger.i(
          '   ✅ Restored cooldown: ${cooldownInfo['minutesRemaining']} minutes remaining');
      return true;
    } else {
  ...
  bool _processProvisionalRecord(
      String studentId, String classId, Map<String, dynamic> record) {
    ...
    _logger.i(
        '   ⏱️ Resuming provisional countdown: ${remainingSeconds}s remaining');

    // Set state to provisional
    _stateManager.setState('provisional',
        studentId: studentId, classId: classId);

    // Restore cooldown using original provisional check-in time if available
    // This prevents duplicate provisional re-entry triggering a full timer reset.
    if (checkInTime != null) {
      _cooldownManager.restoreCooldown(studentId, classId, checkInTime);
    }
    ...
    _confirmationService.scheduleConfirmation(
      ...
      studentId: studentId,
      classId: classId,
      delayOverride: Duration(seconds: remainingSeconds),
    );
