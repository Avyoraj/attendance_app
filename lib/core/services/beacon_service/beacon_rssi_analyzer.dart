...
class BeaconRssiAnalyzer {
  ...
  void feedRssiSample(int rssi) {
    ...
    if (LogConfig.verbose) {
      _logger.d(
          '📥 RSSI sample fed: $rssi dBm (Buffer: ${_rssiSmoothingBuffer.length})');
    }
  }
  ...
  int? getCurrentRssi() {
    ...
    if (timeSinceLastBeacon > AppConstants.beaconLostTimeout) {
      ...
      if (_weakSignalStartTime == null) {
        ...
        if (LogConfig.verbose) {
          _logger.w(
              '⚠️ Beacon weak for ${timeSinceLastBeacon.inSeconds}s - Starting ${AppConstants.exitGracePeriod.inSeconds}s grace period');
          _logger.w(
      ...
      if (weakDuration <= AppConstants.exitGracePeriod) {
        ...
        final remainingSeconds =
            AppConstants.exitGracePeriod.inSeconds - weakDuration.inSeconds;
        if (LogConfig.verbose) {
          _logger.w(
              '⏳ Grace period active: ${remainingSeconds}s remaining (weak for ${weakDuration.inSeconds}s)');
        }
        ...
        _logger.e(
            '❌ Beacon lost for ${weakDuration.inSeconds}s (grace period: ${AppConstants.exitGracePeriod.inSeconds}s)');
        if (LogConfig.verbose) {
    ...
    if (_weakSignalStartTime != null) {
      if (LogConfig.verbose) {
        _logger.i(
            '✅ Beacon signal restored (was weak for ${now.difference(_weakSignalStartTime!).inSeconds}s)');
      }
  ...
  int? _getSmoothedRssi() {
    ...
    final windowSize =
        _rssiSmoothingBuffer.length < AppConstants.rssiSmoothingWindow
            ? _rssiSmoothingBuffer.length
            : AppConstants.rssiSmoothingWindow;

    final recentSamples =
        _rssiSmoothingBuffer.sublist(_rssiSmoothingBuffer.length - windowSize);

    final smoothedRssi = recentSamples.reduce((a, b) => a + b) ~/ windowSize;

    // Throttle RSSI smoothing logs to avoid spam
    if (LogConfig.verbose) {
      // lightweight manual throttle based on modulo of seconds
      final sec = DateTime.now().second;
      if (sec %
              (LogConfig.rssiInterval().inSeconds == 0
                  ? 3
                  : LogConfig.rssiInterval().inSeconds) ==
          0) {
        _logger.d(
            '📊 RSSI Smoothing: Raw=$_currentRssi, Smoothed=$smoothedRssi (avg of $windowSize samples)');
      }
    }
  ...
  Map<String, dynamic> getRawRssiData() {
    ...
    // Calculate RSSI age
    final rssiAge =
        mostRecentTime != null ? now.difference(mostRecentTime) : null;
  ...
  void clearAllData() {
    ...
    if (LogConfig.verbose) _logger.d('🧹 RSSI analyzer data cleared');
  }
