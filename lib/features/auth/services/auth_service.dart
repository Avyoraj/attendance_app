...
class AuthService {
  ...
  final DeviceIdService _deviceIdService = DeviceIdService();  // Use persistent service
  ...
  Future<Map<String, dynamic>> _validateDeviceWithBackend(String studentId, String deviceId) async {
    try {
      final response = await _httpService.post(
        url: '$_baseUrl/validate-device',
        body: {
          'studentId': studentId,
          'deviceId': deviceId,
        },
      ...
      if (response.statusCode == 200) {
        ...
        return {
          ...
          'message': data['message'] ?? 'This device is linked to another account',
  ...
  Future<Map<String, dynamic>> login(String studentId) async {
    ...
    try {
      ...
      print('🔐 LOGIN ATTEMPT:');
      print('   Attempting: $studentId');
      print('   Current Device: $currentDeviceId');
      
      // ✅ CRITICAL: Validate with backend FIRST
      final validationResult = await _validateDeviceWithBackend(studentId, currentDeviceId);
      
      if (validationResult['canLogin'] != true) {
        // Backend blocked login - return detailed error
        print('❌ LOGIN BLOCKED BY BACKEND: ${validationResult['message']}');
        return {
          'success': false,
      ...
      if (studentIdSaved && deviceIdSaved) {
        print('✅ LOGIN SUCCESS: Student $studentId on device $currentDeviceId');
        return {
          'success': true,
